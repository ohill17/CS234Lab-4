class CustomerPage {
  ...
  constructor() {
    ...
    this.server = "https://localhost:44318/api"
  ...
  fetchStates() {
    ...
    //Runs the apiURL with states to retrieve all states
  ...
  onFindCustomer(event) {
    ...
    if (this.$customerId.value != "") {
      ...
      fetch(`${this.url}/${this.state.customerId}`)
        //Runs the apiURL and gets a customer based off its PK 
      .then(response => response.json())
      .then(data => { 
        if (data.status == 404) {
  ...
  //this method controls the deletion of customers
      ...
      fetch(`${this.url}/${this.state.customerId}`, { method: 'DELETE' })
        ...
        .then(data => {
          ...
          if (Object.keys(data).length === 0) {
            //the data returned should be empty json which is a sign its delted succesfully
            //Without checking its length of null status it returns a 404 since it doesnt exist anymore
            //so we check the length to confirm there is nothing returned
            // otherwise youll get a false negative because its not detecting data which is exactly what you do ant
        ...
        .catch(error => {
          alert('There was a problem deleting customer info! ' + error.message);
        });
    } else {
      // Handle the case where customerId is empty
    }
  }
